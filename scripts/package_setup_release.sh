...
#
# Writes: dist/Duke-Nukem-Time-to-Kill-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Duke-Nukem-Time-to-Kill-Recomp \
  --exe-name Duke_Nukem__Time_to_Kill_Recompiled \
  --display-name "Duke Nukem: Time to Kill Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Duke Nukem: Time to Kill disc" \
